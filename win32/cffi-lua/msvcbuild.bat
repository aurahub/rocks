...
:build
if %errorlevel%==0 (cmake -H. -Bbuild %ARG_GENERATOR% %ARG_ARCH% -DCMAKE_BUILD_TARGET=Release)
if %errorlevel%==0 (cmake --build build --config Release --target ffi)
if %errorlevel%==0 (explorer build\lib)
goto :end
